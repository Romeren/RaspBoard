...
class EventDispatcher(object):
    ...
    def dispatch_event(self, event):
        ...
        # print(event.type)
        # print(self._events.keys())
        def dispatch(listeners):
            for listener in listeners:
                listener(event)

        if('*' in self._events.keys()):
            dispatch(self._events['*'])
        if event.type in self._events.keys():
            dispatch(self._events[event.type])
...
class Service_Changed_Event(Event):
    ADDED     = "added_service"
    REMOVED   = "removed_service"
    UPDATED   = "updated_service"
    ERROR     = 'service_error'

class Broker_Event(Event):
    ERROR = 'broker_error'
